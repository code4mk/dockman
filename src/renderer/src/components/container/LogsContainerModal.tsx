...
export default function LogsContainerModal(props): JSX.Element {
  const [processing, setProcessing] = useState(false)
  const [openModal, setOpenModal] = useState(false)
  const [logsData, setLogsData] = useState('')

  useEffect(() => {
    if (props.modalName === 'logsContainerModal') {
      setOpenModal(props.modalStatus)
      if (props?.modalData?.container_id) {
        getLogs(props?.modalData?.container_id)
      }
              ...
              <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 max-w-sm">
                ...
                <div>
                  <div className="mt-4 text-center sm:mt-6">
                    ...
                    <div className="max-h-48 overflow-auto bg-gray-950 text-white">
                      <Ansi>
                        {logsData}
                      </Ansi>;
